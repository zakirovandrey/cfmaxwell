...
enum {IndVac=0, IndSubstrate=1, IndParticle=2};
...
struct parsMaterial {
  ...
  MainType pCx,pCy,pCz,pR,dh,angle;
  void setPars(double dH=0., double ang=0.){
    dh=dH; angle=ang;
    double Xlength=pars.dx*(1<<MaxRank)*pars.Nx, Ylength=pars.dy*(1<<MaxRank)*pars.Ny, Zlength=pars.dz*(1<<MaxRank)*pars.Nz;
    Xbeg=10*pars.dx; Xend=Xlength-10*pars.dx; Ybeg=10*pars.dy; Yend=Ylength-10*pars.dy; Zbeg=-100*pars.dz; Zend=Zlength-0.5;//0.5*Zlength;//1.4;
    ...
    pR=0.065;
    pCx=Xlength*0.5; pCy=Ylength*0.5-sin(angle)*(pR+dh); pCz=Zend+cos(angle)*(pR+dh);
  }
};
