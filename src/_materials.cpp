...
//   if (x>parsMat.Xbeg && x<parsMat.Xend && y>parsMat.Ybeg && y<parsMat.Yend && z>parsMat.Zbeg && z<=parsMat.Zend) return IndMat;
   if (z>parsMat.Zbeg && z<parsMat.Zend+tan(parsMat.angle)*(y-0.5*pars.dy*(1<<MaxRank)*pars.Ny)) return IndSubstrate;
//   if ( fabs(x-parsMat.pCx)<parsMat.pR && fabs(y-parsMat.pCy)<parsMat.pR && fabs(z-parsMat.pCz)<parsMat.pR ) return IndParticle;
//   else if ( (x-parsMat.pCx)*(x-parsMat.pCx)+(y-parsMat.pCy)*(y-parsMat.pCy)+(z-parsMat.pCz)*(z-parsMat.pCz)<=parsMat.pR*parsMat.pR ) return IndParticle;
   else return IndVac;
}
